    ...
    var body: some View {
        if buttonFontLarge == false {
            Button(action: action) {
                Text(buttonText).padding(5).font(.caption2).frame(width: buttonWidth).foregroundColor(foregroundColor).background(backgroundColor).cornerRadius(5).overlay(RoundedRectangle(cornerRadius: 5).stroke(borderColor, lineWidth: 2))
            }
        } else {
            Button(action: action) {
